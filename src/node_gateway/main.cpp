...
namespace {
// The gateway doubles as a small web server and a LoRa sink node.
// It keeps a rolling in-memory packet history so the dashboard can render
// both the latest conditions and short-term trends without cloud storage.
...
String packetHistoryJson() {
  ...
  for (int i = 0; i < stored; ++i) {
    // Return newest packets first so the dashboard can render the latest state
    // without scanning the entire history on the client side.
...
bool sendLoRaMessage(const String &message) {
  ...
  // The LoRa library cannot transmit while in receive mode, so every send
  // explicitly flips the radio to idle and then restores RX afterwards.
  LoRa.idle();
  if (!LoRa.beginPacket()) {
    LoRa.receive();
    return false;
...
void connectToWifiIfNeeded() {
  ...
  // Retry on a timer instead of blocking the main loop so packet reception and
  // HTTP handling keep running even if the access point is unavailable.
...
void readLoRaPackets() {
  ...
  if (extractTaggedId(payload, "MSG:", messageId, appPayload)) {
    // ACKs are generated by the receiver so the sender can tell whether the
    // application payload made it across the radio link.
...
void setup() {
  ...
  if (!setupLoRa()) {
    // The dashboard is still useful for diagnostics even if the radio path is
    // down, so the gateway now stays alive instead of halting in setup().
